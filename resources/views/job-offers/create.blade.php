<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <h1 class="text-xl text-gray-900 dark:text-gray-100">Nouvelle offre</h1>

        <div class="card">
            <form method="POST" action="{{ route('job-offers.store') }}" class="space-y-5" x-data="skillManager()">
                @csrf
                ...
                <div>
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title') }}"
                           ...
                           placeholder="Ex: Développeur Laravel Senior" required>
                    @error('title')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="8"
                              class="block w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-brand-500 focus:ring-brand-500 rounded-xl text-sm resize-y"
                              placeholder="Décrivez le poste, les responsabilités, le contexte…" required>{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <template x-for="(skill, i) in skills" :key="i">
                            <div class="flex gap-2">
                                <input type="text" :name="'required_skills[]'" x-model="skills[i]"
                                       class="flex-1 border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-brand-500 focus:ring-brand-500 rounded-xl text-sm"
                                       placeholder="Ex: Laravel, PHP, MySQL…" required>
                ...
                <div>
                    ...
                    <input id="min_experience" name="min_experience" type="number" value="{{ old('min_experience', 0) }}"
                           class="block w-32 border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-brand-500 focus:ring-brand-500 rounded-xl text-sm"
                           min="0" required>
